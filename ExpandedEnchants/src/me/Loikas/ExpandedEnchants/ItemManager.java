...
public class ItemManager
{
	...
	public ItemMeta AddCustomEnchantmentLore(ItemMeta meta, Enchantment ench) {
		List<String> lore = new ArrayList<String>();
		if(meta.hasLore()) lore = meta.getLore();
		lore.add(ChatColor.GRAY + ench.getName() + " " + Functions.GetNameByLevel(meta.getEnchantLevel(ench), ench.getMaxLevel()));
		meta.setLore(lore);
		return meta;
	}
	...
	@SuppressWarnings("deprecation")
	public ItemMeta RemoveCustomEnchantmentLore(ItemMeta meta, Enchantment ench) {
		List<String> lore = new ArrayList<String>();
		if(meta.hasLore()) lore = meta.getLore();
		else return meta;
		for (int i = 0; i < lore.size(); i++)
		{
			if (lore.get(i).contains(ench.getName()))
			{
				lore.remove(i);
			}
		}
	...
	public ItemStack CreateCustomBook(Enchantment ench, int level) {
		...
		List<String> lore = new ArrayList<String>();
		lore.add(ChatColor.GRAY + ench.getName() + " " + Functions.GetNameByLevel(item.getEnchantmentLevel(ench), ench.getMaxLevel()));
		meta.setLore(lore);
	...
	public ItemStack SoulboundTotem(int level) {
		ItemStack totem = new ItemStack(Material.HEART_OF_THE_SEA, 1);
		ItemMeta meta = totem.getItemMeta();
		meta.setDisplayName("Soulbound Totem " + Functions.GetNameByLevel(level, 2));
		List<String> lore = new ArrayList<>();
		lore.add(ChatColor.GRAY + "Lets you keep your items on death");
		if(level == 2) lore.add(ChatColor.GRAY + "Lets you keep your xp on death");
		lore.add("ONE USE ONLY");
		lore.add("Hold in your offhand or mainhand when dying to use");
		meta.setLore(lore);
		meta.addEnchant(CustomEnchantsManager.NOBREAKABLE, 1, true);
	...
	public ItemStack GetSpecialCraftItem(ShapedRecipe recipe) {
		...
		switch(recipe.getKey().toString()) {
			case "minecraft:ee_recipe_flamingfall":
			{
				...
				ItemStack item = GetConfigItems("minecraft:ee_recipe_flamingfall");
