...
{	
	...
	public void onEntityDamageEvent(EntityDamageEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("DeflectEnabled")) HandleDeflectEnchant(e);
	}

	@EventHandler
	public void onEntityShootBowEvent(EntityShootBowEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("AntigravityEnabled"))HandleAntiGravityEnchant(e);
	}

	@EventHandler()
	public void onBlockBreakEvent(BlockBreakEvent event) {
		if(Main.getPlugin().getConfig().getBoolean("LumberjackEnabled")) HandleLumberjackEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("VeinmineEnabled")) HandleVeinmineEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("WideEnabled")) HandleWideEnchant(event);
	}

	@EventHandler
	public void onEntityDeathEvent(EntityDeathEvent event) {
		if(Main.getPlugin().getConfig().getBoolean("BeheadingEnabled")) HandleBeheadingEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("DirectEnabled")) HandleDirectEntityEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("LifestealEnabled")) HandleLifestealEnchant(event);
	}

	@EventHandler
	public void onBlockDropItemEvent(BlockDropItemEvent event) {
		if(Main.getPlugin().getConfig().getBoolean("AutosmeltEnabled")) HandleAutoSmeltEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("DirectEnabled")) HandleDirectBlockEnchant(event);
		if(Main.getPlugin().getConfig().getBoolean("ReplantingEnabled"))CheckReplantingItemDrop(event);
	}
	...
	public void onPlayerExpChangeEvent(PlayerExpChangeEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("ExperienceboostEnabled")) HandleExpBoostEnchant(e);
	}

	@EventHandler (priority = EventPriority.HIGH)
	public void onPlayerMoveEvent(PlayerMoveEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("LavawalkerEnabled"))HandleLavaWalkerEnchant(e);
	}

	@EventHandler
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("IcyEnabled")) HandleIcyEnchant(e);
	}
	...
	public void onFoodLevelChangeEvent(FoodLevelChangeEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("FeedingmoduleEnabled")) HandleFeedingModuleEnchant(e);
	}
	
	@EventHandler
	public void onEntityTargetEvent(EntityTargetEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("ShadowstepEnabled"))HandleShadowStepEnchant(e);
	}

	@EventHandler
	public void onPlayerInteractEvent(PlayerInteractEvent e) {
		if(Main.getPlugin().getConfig().getBoolean("ReplantingEnabled"))CheckReplantingEnchant(e);
		if(Main.getPlugin().getConfig().getBoolean("WideEnabled"))HandleWideSpecialActionEnchant(e);
	}
	...
	public void CheckWanderingTrader(EntitySpawnEvent e) {
		...
		if(ran > Main.getPlugin().getConfig().getInt("WanderingTraderChance")) return;
	...
	public void CheckReplantingEnchant(PlayerInteractEvent e) {
		...
		if(!e.getItem().getItemMeta().hasEnchant(CustomEnchantsManager.REPLANTING)) return;
		...
		if(data instanceof Ageable) {
			...
			if(ageData.getAge() != ageData.getMaximumAge()) return;
	...
	public void CheckReplantingItemDrop(BlockDropItemEvent e) {
		...
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.REPLANTING)) return;
		BlockData data = e.getBlock().getBlockData();
		if(data == null) return;
		if(data.getMaterial() == Material.AIR) data = replantingBlock; 
		if(data instanceof Ageable) {
			Ageable ageData = (Ageable) data;
			if(ageData.getAge() != ageData.getMaximumAge()) return;
			...
			if(data.getMaterial() == Material.CARROTS) {
				for(Item item : items) if(item.getItemStack().getType().equals(Material.CARROT)) {
					item.getItemStack().setAmount(item.getItemStack().getAmount() - 1);
					break;
				}
			}
			if(data.getMaterial() == Material.POTATOES) {
				for(Item item : items) if(item.getItemStack().getType().equals(Material.POTATO)) {
					item.getItemStack().setAmount(item.getItemStack().getAmount() - 1);
					break;
				}
			}
			if(data.getMaterial() == Material.BEETROOTS) {
				for(Item item : items) if(item.getItemStack().getType().equals(Material.BEETROOT)) {
					item.getItemStack().setAmount(item.getItemStack().getAmount() - 1);
					break;
				}
			}
			if(data.getMaterial() == Material.NETHER_WART) {
				for(Item item : items) if(item.getItemStack().getType().equals(Material.NETHER_WART)) {
					item.getItemStack().setAmount(item.getItemStack().getAmount() - 1);
					break;
				}
			}
	...
	public void CheckForDiscoverRecipes(CraftItemEvent e) {
		...
		if(e.getInventory().getResult().getType() == Material.CRAFTING_TABLE) {
			...
			for(CustomEnchantmentRecipe recipe : Main.customRecipes) keys.add(recipe.recipe.getKey());
	...
	public void CheckForCustomRecipes(PrepareItemCraftEvent e) {
		...
		if(((Keyed) recipe).getKey() == null) return;

		CustomEnchantmentRecipe customRecipe = null;
		for(CustomEnchantmentRecipe cer : Main.customRecipes) { 
			if(cer.recipe.getKey().equals(((Keyed) recipe).getKey())) customRecipe = cer; }
		...
		if(!canCraft) inv.setResult(null);
	...
	public void HandleShadowStepEnchant(EntityTargetEvent e) {
		if(e.getTarget() instanceof Player) {
			...
			if(player.getInventory().getBoots().getItemMeta() == null) return;
			if(!player.getInventory().getBoots().getItemMeta().hasEnchant(CustomEnchantsManager.SHADOWSTEP)) return;
			
			int level = player.getInventory().getBoots().getItemMeta().getEnchantLevel(CustomEnchantsManager.SHADOWSTEP);
	...
	public void HandleCraftedItem(CraftItemEvent e) {
		...
		for(int i = 0; i < customRecipe.amounts.length; i++) {
			...
			if(leftOverAmount == 0) leftOvers[i] = null;
			else leftOvers[i].setAmount(leftOverAmount);
		}
	...
	public void CountdownDeflectEnchant() {
		for(Player player : deflectCountdown.keySet()) {
			...
			if(oldValue > 0) deflectCountdown.put(player, oldValue - 1);
			else oldValue = 0;
	...
	public void HandleDeflectEnchant(EntityDamageEvent e) {
		if(e.getEntity() instanceof Player) {
			...
			if(!player.getInventory().getChestplate().getItemMeta().hasEnchant(CustomEnchantsManager.DEFLECT)) return;
			...
			if(!deflectCountdown.containsKey(player)) {
				...
				player.spigot().sendMessage(ChatMessageType.ACTION_BAR, component);
				player.getInventory().removeItem(new ItemStack(Material.EMERALD, Main.getPlugin().getConfig().getInt("DeflectCost")));
			}
				...
				player.spigot().sendMessage(ChatMessageType.ACTION_BAR);
				TextComponent component = new TextComponent("Deflect has a cooldown left of " + deflectCountdown.get(player) + " seconds!");
				component.setColor(ChatColor.RED);
				player.spigot().sendMessage(ChatMessageType.ACTION_BAR, component);
				return;
			}
			deflectCountdown.put(player, 31 - player.getInventory().getChestplate().getItemMeta().getEnchantLevel(CustomEnchantsManager.DEFLECT));
		}
	}
	...
	public void HandleThermalPlatingEnchant(Player player) {
		if(player != null) {
			if(player.getInventory().getChestplate() != null && player.getInventory().getHelmet() != null && player.getInventory().getBoots() != null && player.getInventory().getLeggings() != null) {
				if(player.getInventory().getChestplate().hasItemMeta() && player.getInventory().getHelmet().hasItemMeta() && player.getInventory().getBoots().hasItemMeta() && player.getInventory().getLeggings().hasItemMeta()) {
					if(player.getInventory().getChestplate().getItemMeta().hasEnchant(CustomEnchantsManager.THERMALPLATING) && player.getInventory().getHelmet().getItemMeta().hasEnchant(CustomEnchantsManager.THERMALPLATING) && player.getInventory().getBoots().getItemMeta().hasEnchant(CustomEnchantsManager.THERMALPLATING) && player.getInventory().getLeggings().getItemMeta().hasEnchant(CustomEnchantsManager.THERMALPLATING)) {
						PotionEffect effect = new PotionEffect(PotionEffectType.FIRE_RESISTANCE, 99999999, 0, false, false, false);
						if(!player.hasPotionEffect(PotionEffectType.FIRE_RESISTANCE)) player.addPotionEffect(effect);
						if(!thermalPlayers.contains(player)) thermalPlayers.add(player);
						return;
					}
				}
			}
		...
		if(player != null) {
			if(player.getInventory().getChestplate() != null) {
				if(player.getInventory().getChestplate().hasItemMeta()) {
					if(player.getInventory().getChestplate().getItemMeta().hasEnchant(CustomEnchantsManager.HEAVENSLIGHTNESS)) {
						PotionEffect effect = new PotionEffect(PotionEffectType.SLOW_FALLING, 99999999, 0, false, false, false);
						if(!player.hasPotionEffect(PotionEffectType.SLOW_FALLING)) player.addPotionEffect(effect);
						if(!heavensPlayers.contains(player)) heavensPlayers.add(player);
						return;
					}
				}
			}
			...
			if(player.getInventory().getHelmet() != null) {
				if(player.getInventory().getHelmet().hasItemMeta()) {
					if(player.getInventory().getHelmet().getItemMeta().hasEnchant(CustomEnchantsManager.OWLEYES)) {
						PotionEffect effect = new PotionEffect(PotionEffectType.NIGHT_VISION, 99999999, 0, false, false, false);
						if(!player.hasPotionEffect(PotionEffectType.NIGHT_VISION)) player.addPotionEffect(effect);
						if(!owleyesPlayers.contains(player)) owleyesPlayers.add(player);
						return;
					}
				}
			}
			...
			if(player.getInventory().getBoots() != null) {
				if(player.getInventory().getBoots().hasItemMeta()) {
					if(player.getInventory().getBoots().getItemMeta().hasEnchant(CustomEnchantsManager.LEAPING)) {
						PotionEffect effect = new PotionEffect(PotionEffectType.JUMP, 999999, player.getInventory().getBoots().getItemMeta().getEnchantLevel(CustomEnchantsManager.LEAPING), false, false, false);
						player.removePotionEffect(PotionEffectType.JUMP);
						player.addPotionEffect(effect);
						if(!leapingPlayers.contains(player)) leapingPlayers.add(player);
						return;
					}
				}
			}
	...
	public void HandleIcyEnchant(EntityDamageByEntityEvent e) {
		if(e.getEntity() instanceof LivingEntity) {
			...
			if(e.getDamager() instanceof Player) {
				...
				if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.ICY)) return;
				if(player.getGameMode() == GameMode.SPECTATOR) return;	
	
				int level = player.getInventory().getItemInMainHand().getItemMeta().getEnchantLevel(CustomEnchantsManager.ICY) * 20;
	...
	public void HandleLumberjackEnchant(BlockBreakEvent e) {
		if(veinMinedBlocks.contains(e.getBlock())) return; 
		Block originBlock = e.getBlock();
		if(!EventsClass.functions.IsLumberjackBlock(originBlock.getType())) return;
		Player player = e.getPlayer();
		if(player == null) return;
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.LUMBERJACK)) return;
		if(player.getGameMode() == GameMode.SPECTATOR) return;	
		int maxMineSize = 128;
		...
		if(!itemMeta.isUnbreakable()) {
			if(itemMeta.hasEnchant(Enchantment.DURABILITY)) itemMeta.setDamage(itemMeta.getDamage() + (checkedBlocks.size() / itemMeta.getEnchantLevel(Enchantment.DURABILITY) + 1));
			else itemMeta.setDamage(itemMeta.getDamage() + checkedBlocks.size());
		}
		...
		if(veinMinedBlocks.size() > 0) e.setDropItems(false);
	...
	public void HandleVeinmineEnchant(BlockBreakEvent e) {
		if(veinMinedBlocks.contains(e.getBlock())) return; 
		Block originBlock = e.getBlock();
		if(!EventsClass.functions.IsVeinmineBlock(originBlock.getType())) return;
		Player player = e.getPlayer();
		if(player == null) return;
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.VEINMINE)) return;
		if(player.getGameMode() == GameMode.SPECTATOR) return;	
		int maxMineSize = 128;
		Material mat = originBlock.getType();
		checkedBlocks.clear();
	...
	public void CheckSurroundingBlocks(Block block, int maxSize)
	{
		...
		if(checkedBlocks.size() == maxSize) return;
		Location under = new Location(blockLoc.getWorld(), blockLoc.getX(), blockLoc.getY() - 1, blockLoc.getZ());
		Block underBlock = under.getBlock();
		if(underBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(underBlock)) {
			checkedBlocks.add(underBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(underBlock, maxSize);
			else return;
		}
		Location up = new Location(blockLoc.getWorld(), blockLoc.getX(), blockLoc.getY() + 1, blockLoc.getZ());
		Block upBlock = up.getBlock();
		if(upBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(upBlock)) {
			checkedBlocks.add(upBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(upBlock, maxSize);
			else return;
		}
		Location north = new Location(blockLoc.getWorld(), blockLoc.getX(), blockLoc.getY(), blockLoc.getZ() - 1);
		Block northBlock = north.getBlock();
		if(northBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(northBlock)) {
			checkedBlocks.add(northBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(northBlock, maxSize);
			else return;
		}
		Location south = new Location(blockLoc.getWorld(), blockLoc.getX(), blockLoc.getY(), blockLoc.getZ() + 1);
		Block southBlock = south.getBlock();
		if(southBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(southBlock)) {
			checkedBlocks.add(southBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(southBlock, maxSize);
			else return;
		}
		Location east = new Location(blockLoc.getWorld(), blockLoc.getX() + 1, blockLoc.getY(), blockLoc.getZ());
		Block eastBlock = east.getBlock();
		if(eastBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(eastBlock)) {
			checkedBlocks.add(eastBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(eastBlock, maxSize);
			else return;
		}
		Location west = new Location(blockLoc.getWorld(), blockLoc.getX() - 1, blockLoc.getY(), blockLoc.getZ());
		Block westBlock = west.getBlock();
		if(westBlock.getType().equals(block.getType())) if(!checkedBlocks.contains(westBlock)) {
			checkedBlocks.add(westBlock);
			if(checkedBlocks.size() < maxSize) CheckSurroundingBlocks(westBlock, maxSize);
			else return;
		}
	}		
	...
	public void HandleWideEnchant(BlockBreakEvent e) {
		...
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.WIDE)) return;
		if(player.getGameMode() == GameMode.SPECTATOR) return;	
		...
		Damageable itemMeta = (Damageable) player.getInventory().getItemInMainHand().getItemMeta();
		if(!itemMeta.isUnbreakable()) {
			if(itemMeta.hasEnchant(Enchantment.DURABILITY)) itemMeta.setDamage(itemMeta.getDamage() + (blocks.size() / itemMeta.getEnchantLevel(Enchantment.DURABILITY) + 1));
			else itemMeta.setDamage(itemMeta.getDamage() + blocks.size());
		}
	...
	public void HandleWideSpecialActionEnchant(PlayerInteractEvent e) {
			...
			if(mat == Material.DIRT || mat == Material.GRASS_BLOCK || mat == Material.DIRT_PATH) {
				...
				for(Block block : blocks) {
					...
					if(bmat == Material.DIRT || bmat == Material.GRASS_BLOCK || bmat == Material.DIRT_PATH) {
						if(new Location(block.getWorld(), block.getX(), block.getY() + 1, block.getZ()).getBlock().getType() == Material.AIR) {
		...
		if(functions.CheckAxeTypes(item)) {
			if(!functions.CheckNonStrippedLogTypes(e.getClickedBlock().getType())) return;
		...
		if(functions.CheckShovelTypes(item)) {
			Material mat = e.getClickedBlock().getType();
			if(mat == Material.DIRT || mat == Material.GRASS_BLOCK || mat == Material.COARSE_DIRT || mat == Material.MYCELIUM || mat == Material.PODZOL || mat == Material.ROOTED_DIRT) {
				ArrayList<Block> blocks = GetSurroundingWideBlocks(e.getClickedBlock(), BlockFace.UP);
				for(Block block : blocks) {
					Material bmat = block.getType();
					if(bmat == Material.DIRT || bmat == Material.GRASS_BLOCK || bmat == Material.COARSE_DIRT || bmat == Material.MYCELIUM || bmat == Material.PODZOL || bmat == Material.ROOTED_DIRT) {
						if(new Location(block.getWorld(), block.getX(), block.getY() + 1, block.getZ()).getBlock().getType() == Material.AIR) {
	...
	public ArrayList<Block> CheckWideBlocks(Block block, BlockFace face) {
		...
		if(face.equals(BlockFace.UP) || face.equals(BlockFace.DOWN)) {
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() + 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ() + 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ() + 1).getBlock());
		}
		if(face.equals(BlockFace.SOUTH) || face.equals(BlockFace.NORTH)) {
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY(), loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY(), loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY() + 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY() + 1, loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY() - 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY() - 1, loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() + 1, loc.getY() - 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() + 1, loc.getY() - 1, loc.getZ()).getBlock());
			if(new Location(loc.getWorld(), loc.getX() - 1, loc.getY() + 1, loc.getZ()).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX() - 1, loc.getY() + 1, loc.getZ()).getBlock());
		}
		if(face.equals(BlockFace.EAST) || face.equals(BlockFace.WEST)) {
			...
			if(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() + 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY(), loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ() + 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ() - 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() + 1, loc.getZ() - 1).getBlock());
			if(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ() + 1).getBlock().getType().equals(block.getType())) blocks.add(new Location(loc.getWorld(), loc.getX(), loc.getY() - 1, loc.getZ() + 1).getBlock());
		}
	...
	public void HandleFeedingModuleEnchant(FoodLevelChangeEvent e) {
		if(e.getEntity() instanceof Player) {
			...
			if(!player.getInventory().getHelmet().getItemMeta().hasEnchant(CustomEnchantsManager.FEEDINGMODULE)) return;
			if(player.getGameMode() == GameMode.SPECTATOR || player.getGameMode() == GameMode.CREATIVE) return;	
			if(!functions.PlayerContainsFoodItems(player)) return;
			ItemStack item = functions.GetFirstFoodItem(player);
			if(item == null) return; 
			int foodValue = functions.GetFoodPoints(item.getType());
	...
	public void HandleLifestealEnchant(EntityDeathEvent e) {
		...
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.LIFESTEAL)) return;
		if(player.getGameMode() == GameMode.SPECTATOR || player.getGameMode() == GameMode.CREATIVE) return;
		int level = player.getInventory().getItemInMainHand().getItemMeta().getEnchantLevel(CustomEnchantsManager.LIFESTEAL);
		double health = player.getHealth();
		double healing = 1 * level;
		double total = health + healing;
		if(total > player.getMaxHealth()) total = player.getMaxHealth();
	...
	public void HandleLavaWalkerEnchant(PlayerMoveEvent e) {
		...
		if(!player.getInventory().getBoots().getItemMeta().hasEnchant(CustomEnchantsManager.LAVA_WALKER)) return;
	...
	public void HandleExpBoostEnchant(PlayerExpChangeEvent e) {
		...
		if(player.getInventory().getChestplate() == null) return;
		if(!player.getInventory().getChestplate().hasItemMeta()) return;
		if(!player.getInventory().getChestplate().getItemMeta().hasEnchant(CustomEnchantsManager.EXP_BOOST)) return;
		if(player.getGameMode() == GameMode.SPECTATOR || player.getGameMode() == GameMode.CREATIVE) return;

		double chance = player.getInventory().getChestplate().getItemMeta().getEnchantLevel(CustomEnchantsManager.EXP_BOOST) * 0.15;
	...
	public void HandleBeheadingEnchant(EntityDeathEvent e) {
		...
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.BEHEADING)) return;
		if(player.getGameMode() == GameMode.SPECTATOR) return;	

		double chance = player.getInventory().getItemInMainHand().getItemMeta().getEnchantLevel(CustomEnchantsManager.BEHEADING) * 0.05;
		double random = Math.random();

		if(random < chance) {
			if(mob.getType() == EntityType.ZOMBIE || mob.getType() == EntityType.ZOMBIE_VILLAGER || mob.getType() == EntityType.HUSK) e.getDrops().add(new ItemStack(Material.ZOMBIE_HEAD));
			if(mob.getType() == EntityType.SKELETON || mob.getType() == EntityType.STRAY) e.getDrops().add(new ItemStack(Material.SKELETON_SKULL));
			if(mob.getType() == EntityType.WITHER_SKELETON) e.getDrops().add(new ItemStack(Material.WITHER_SKELETON_SKULL));
			if(mob.getType() == EntityType.CREEPER) e.getDrops().add(new ItemStack(Material.CREEPER_HEAD));
			if(mob.getType() == EntityType.ENDER_DRAGON) e.getDrops().add(new ItemStack(Material.DRAGON_HEAD));
		}
	}
	...
	public void HandleAntiGravityEnchant(EntityShootBowEvent e) {
		if(e.getEntity() instanceof Player) {
			...
			if(player.getInventory().getItemInMainHand() == null) return;
			if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
			if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.ANTIGRAVITY)) return;
	...
	public void HandleDirectEntityEnchant(EntityDeathEvent e) {
		...
		if(player.getInventory().getItemInMainHand() == null) return;
		if(!player.getInventory().getItemInMainHand().hasItemMeta()) return;
		if(!player.getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) return;
	...
	public void HandleDirectBlockEnchant(BlockDropItemEvent event) {
		...
		if(!event.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) return;
		...
		if(!event.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.AUTOSMELT)) {
	...
	public void HandleAutoSmeltEnchant(BlockDropItemEvent e) {
		...
		if(!e.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.AUTOSMELT)) return;
		...
		boolean isOre = false;
		for(Item item : items) {
			if(item.getItemStack().getType() == Material.RAW_IRON || item.getItemStack().getType() == Material.RAW_GOLD || item.getItemStack().getType() == Material.RAW_COPPER || item.getItemStack().getType() == Material.ANCIENT_DEBRIS) isOre = true;
		}
		if(!isOre) {
		...
		for(Item item : items) {
			...
			if(functions.CheckPickaxeTypes(e.getPlayer().getInventory().getItemInMainHand())) {
				if(item.getItemStack().getType() == Material.RAW_IRON) {
					if(e.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) e.getPlayer().getInventory().addItem(new ItemStack(Material.IRON_INGOT, item.getItemStack().getAmount()));
					else e.getBlock().getLocation().getWorld().dropItemNaturally(e.getBlock().getLocation(), new ItemStack(Material.IRON_INGOT, item.getItemStack().getAmount()));
					i++;
				}
				if(item.getItemStack().getType() == Material.RAW_GOLD) {
					if(e.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) e.getPlayer().getInventory().addItem(new ItemStack(Material.GOLD_INGOT, item.getItemStack().getAmount()));
					else e.getBlock().getLocation().getWorld().dropItemNaturally(e.getBlock().getLocation(), new ItemStack(Material.GOLD_INGOT, item.getItemStack().getAmount()));
					i++;
				}
				if(item.getItemStack().getType() == Material.RAW_COPPER) {
					if(e.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) e.getPlayer().getInventory().addItem(new ItemStack(Material.COPPER_INGOT, item.getItemStack().getAmount()));
					else e.getBlock().getLocation().getWorld().dropItemNaturally(e.getBlock().getLocation(), new ItemStack(Material.COPPER_INGOT, item.getItemStack().getAmount()));
					i++;
				}
				if(item.getItemStack().getType() == Material.ANCIENT_DEBRIS) {
					if(e.getPlayer().getInventory().getItemInMainHand().getItemMeta().hasEnchant(CustomEnchantsManager.DIRECT)) e.getPlayer().getInventory().addItem(new ItemStack(Material.NETHERITE_SCRAP, item.getItemStack().getAmount()));
					else e.getBlock().getLocation().getWorld().dropItemNaturally(e.getBlock().getLocation(), new ItemStack(Material.NETHERITE_SCRAP, item.getItemStack().getAmount()));
					i++;
				}
			}
		}
	...
	public void HandleDisenchant(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		...
		if(fs.getType() == Material.ENCHANTED_BOOK) {
			if(functions.ContainsCustomEnchant(fs)) return;
		...
		else 
		{
			...
			if(functions.IsCustomEnchant(removeEnchant)) {
				...
				lore.add(ChatColor.GRAY + removeEnchant.getName() + " " + functions.GetNameByLevel(fs.getEnchantmentLevel(removeEnchant), removeEnchant.getMaxLevel()));
	...
	public void HandleBookEnchant(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		if(ss.getType() != Material.ENCHANTED_BOOK || fs.getType() == Material.ENCHANTED_BOOK) return;

		if(functions.ContainsCustomEnchant(ss)) return;
		...
		for(Enchantment item : meta.getStoredEnchants().keySet()) {	
			for(Enchantment item2 : resultMeta.getEnchants().keySet()) {
				...
				if(item.conflictsWith(item2)) return;
			}
	...
	public void HandleBookMerge(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		if(fs.getType() != Material.ENCHANTED_BOOK || ss.getType() != Material.ENCHANTED_BOOK) return;
		if(functions.ContainsCustomEnchant(fs) && functions.ContainsCustomEnchant(ss)) {
			...
			if(!ench.equals(ss.getEnchantments().keySet().toArray()[0])) return;
			int highestlevel = fs.getItemMeta().getEnchantLevel(ench) >= ss.getItemMeta().getEnchantLevel(ench) ? fs.getItemMeta().getEnchantLevel(ench) : ss.getItemMeta().getEnchantLevel(ench);
			ItemStack item = new ItemStack(fs);
			if(fs.getItemMeta().getEnchantLevel(ench) == ss.getItemMeta().getEnchantLevel(ench)) highestlevel = fs.getItemMeta().getEnchantLevel(ench) + 1;
			if(highestlevel > ench.getMaxLevel()) return;
			item.addUnsafeEnchantment(ench, highestlevel);
			ItemMeta meta = item.getItemMeta();
			List<String> lore = new ArrayList<String>();
			lore.add(ChatColor.GRAY + ench.getName() + " " + functions.GetNameByLevel(item.getItemMeta().getEnchantLevel(ench), ench.getMaxLevel()));			
			meta.setLore(lore);
			meta.setDisplayName(e.getInventory().getRenameText());
	...
	public void HandleCustomEnchantBooks(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		if(ss.getType() != Material.ENCHANTED_BOOK) return;
		if(fs.getType() == Material.ENCHANTED_BOOK) return;
		if(!functions.ContainsCustomEnchant(ss)) return;
		...
		if(fs.getItemMeta().hasEnchant(ench)) {
			if(fs.getItemMeta().getEnchantLevel(ench) > level) return;
			if(ench.getMaxLevel() == 1) return;
			ItemStack resultItem = new ItemStack(fs);
			int addLevel = fs.getItemMeta().getEnchantLevel(ench) == level ? level + 1 : level;
			if(addLevel > ench.getMaxLevel()) return;
		...
		if(!ench.canEnchantItem(fs)) return;
		if(fs.getItemMeta().hasConflictingEnchant(ench)) return;
	...
	public void HandleSameItemUpgrades(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		if(fs.getType() != ss.getType()) return;
		if(fs.getType() == Material.ENCHANTED_BOOK) return;
		...
		for(Object ench : ssEnchants) 
		{
			...
			if(fs.getItemMeta().hasEnchant(enc)) 
			{
				...
				if(functions.IsCustomEnchant(enc)) {
					...
					if(ssLevel == fsLevel) {
						if(ssLevel == enc.getMaxLevel()) continue;
						else {
				...
				if(functions.IsCustomEnchant(enc)) {
					...
					if(meta.hasLore()) lore = meta.getLore();
					int num = -1;
					for(int i = 0; i < lore.size(); i++) if(lore.get(i).contains(enc.getName())) num = i;						
					lore.remove(num);
					lore.add(ChatColor.GRAY + enc.getName() + " " + functions.GetNameByLevel(resultItem.getEnchantmentLevel(enc), enc.getMaxLevel()));
					meta.setLore(lore);
					meta.setDisplayName(e.getInventory().getRenameText());
					resultItem.setItemMeta(meta);
				}
				...
				if(!fs.getItemMeta().hasConflictingEnchant(enc)) {
					...
					if(functions.IsCustomEnchant(enc)) {
						...
						if(resultItem.getItemMeta().hasEnchant(CustomEnchantsManager.TRAVELER)) {
							ItemMeta itemMeta = resultItem.getItemMeta();
							itemMeta.addAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED, new AttributeModifier("generic.movementSpeed", itemMeta.getEnchantLevel(CustomEnchantsManager.TRAVELER) * .4, AttributeModifier.Operation.ADD_SCALAR));
							itemMeta.addItemFlags(ItemFlag.HIDE_ATTRIBUTES);
							resultItem.setItemMeta(itemMeta);
						}
						if(resultItem.getItemMeta().hasEnchant(CustomEnchantsManager.STONEFISTS)) {
	...
	public void HandleResourceEnchants(ItemStack fs, ItemStack ss, PrepareAnvilEvent e) {
		...
		if (goal == null) return;
		if (!goal.canEnchantItem(fs)) return;
		if (ss.getAmount() < Main.recipeManager.GetAmount(goal)) return;
		...
		int lvl = 1;
		int extra = 0;
		if(fs.getItemMeta().hasEnchant(goal)) extra = meta.getEnchantLevel(goal);
		int tot = lvl + extra;
						...
						if(rawSlot == 2)
						{
							...
							if(item != null)
							{
								...
								if(meta != null)
								{
									...
									if(ainv.getRepairCost() > player.getLevel()) return;
										...
										if(removeLastEnchant) {
											if(firstItem.getType() == Material.ENCHANTED_BOOK) {
												...
												if(functions.IsCustomEnchant(removeEnch)) {
													...
													if(removeEnch.equals(CustomEnchantsManager.NOBREAKABLE)) enchmeta.setUnbreakable(false);
													if(removeEnch.equals(CustomEnchantsManager.TRAVELER)) enchmeta.removeAttributeModifier(Attribute.GENERIC_MOVEMENT_SPEED);
													if(removeEnch.equals(CustomEnchantsManager.HEALTHBOOST)) enchmeta.removeAttributeModifier(Attribute.GENERIC_MAX_HEALTH);
													if(removeEnch.equals(CustomEnchantsManager.STONEFISTS)) enchmeta.removeAttributeModifier(Attribute.GENERIC_ATTACK_DAMAGE);
